...
var _paypalCompleteJS = function() {
	...
	this.loadScript = function(token) {
		...
		// 	url += '&vault=true';
		// }
		var disabledAPMList = ['sofort', 'sepa', 'p24', 'mybank', 'mercadopago', 'ideal', 'giropay', 'eps', 'blik', 'bancontact', 'credit'];
		...
		if (self.state.renderSPB === 'true' && self.state.renderCardFields !== 'true') {
			...
			// adds to disable-funding
			if (self.state.nameAPM === 'Venmo') {
				url += ',' + disabledAPMList.join(',');
			}
	...
	this.paymentMethodsEventHandlers = function() {
		...
		window.onload = function() {
			var isOnlySpb = self.state.renderSPB === 'true' && self.state.renderCardFields === 'false';
			var isOnlyCardFields = self.state.renderCardFields === 'true' && self.state.renderSPB === 'false';
			var isBothEnabled = self.state.renderCardFields === 'true' && self.state.renderSPB === 'true';
			var paypalTypeClassName;

			if (isOnlySpb || isOnlyCardFields) {
				if (isOnlySpb) {
	...
	this.paypalUtils = {
		getDataFields: function() {
			this.state = {
				...
				renderSPB: wrapper ? (!!wrapper.getAttribute('data-show-spb')).toString() : false, // undefined check
		...
		getOrdersData: function() {
			...
			var isStoreBuilder = window.JFAppsManager && window.JFAppsManager.checkoutKey;
			var total = isStoreBuilder ? window.JFAppsManager.cartTotal : utils.getPrice('total');
			var item_total = isStoreBuilder ? window.JFAppsManager.cartTotal : utils.getPrice('item_total');
